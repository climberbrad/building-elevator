import {findClosest, travelTime, quickSort, bubbleSort, getElevatorStat, ElevatorStat} from "./util/ElevatorUtil.ts";
...
test('travel time', () => {
    ...
    // (12 - 6) * 10)) + (12 - 15) * 10)) = 90
    const expected = 90;
    expect(travelTime(currentFloor, destinations, timePerFloor)).toEqual(expected)
})
...
test('total time bubble sort', () => {
    const stat: ElevatorStat = getElevatorStat(1, [9, 6, 4, 2], bubbleSort, TRAVEL_TIME_PER_FLOOR)
    expect(stat).toEqual({currentFloor: 1, destinations: [2, 4, 6, 9], totalTime: 80})
    console.log(stat)
})

test('total time quick sort', () => {
    const stat: ElevatorStat = getElevatorStat(1, [9, 6, 4, 2], quickSort, TRAVEL_TIME_PER_FLOOR)
    expect(stat).toEqual({currentFloor: 1, destinations: [2, 4, 6, 9], totalTime: 80})
    console.log(stat)
})

test('total time quick sort full list', () => {
    const stat: ElevatorStat = getElevatorStat(1, DESTINATIONS_UNSORTED, quickSort, TRAVEL_TIME_PER_FLOOR)
    expect(stat).toEqual({currentFloor: 1, destinations: DESTINATIONS_SORTED, totalTime: 230})
    console.log(stat)
})

test('total time bubble sort full list', () => {
    const stat: ElevatorStat = getElevatorStat(1, DESTINATIONS_UNSORTED, bubbleSort, TRAVEL_TIME_PER_FLOOR)
    expect(stat).toEqual({currentFloor: 1, destinations: DESTINATIONS_SORTED, totalTime: 230})
    console.log(stat)
})
