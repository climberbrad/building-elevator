...
function binarySearch(current: number, orderedList: number[], tieGoesToHigherNumber?: boolean): number[] {
    const middleIndex: number = Math.floor(orderedList.length / 2);

    if (orderedList.length === 2) {

        // In a tie: default to the lower number if tieGoesToHigherNumber is not provided
        if (Math.abs(current - orderedList[0]) === Math.abs(current - orderedList[1])) {
            return tieGoesToHigherNumber ? [orderedList[1]] : [orderedList[0]]
        }

        // return the closest number as an array with length 1
    ...
    if (orderedList.length > 2) {
        if (current < orderedList[middleIndex]) {
            return binarySearch(current, orderedList.slice(0, middleIndex), tieGoesToHigherNumber)
        } else if (current > orderedList[middleIndex]) {
            return binarySearch(current, orderedList.slice(middleIndex, orderedList.length), tieGoesToHigherNumber)
        }
    }
...
export const closestTo = (current: number, unorderedList: number[], tieGoesToHigherNumber?: boolean): number => {
    ...
    const result: number[] = binarySearch(current, [...sortedList], tieGoesToHigherNumber)
